...
public class CarouselServiceImpl implements CarouselService {
    ...
    public ResultVO add(Carousel carousel) {
        ...
        int affected = carouselMapper.insertSelective(carousel);
        ResultVO resultVO;
        if (affected > 0) {
    ...
    public ResultVO update(Carousel carousel) {
        int affected = carouselMapper.updateByPrimaryKeySelective(carousel);
        ResultVO resultVO;
        if (affected > 0) {
    ...
    public ResultVO delete(Integer id) {
        int affected = carouselMapper.deleteByPrimaryKey(id);
        ResultVO resultVO;
        if (affected > 0) {
    ...
    public ResultVO changeStatus(Integer id) {
        ...
        int affected = carouselMapper.changeStatus(id);
        ResultVO resultVO;
        if (affected > 0) {
            resultVO = new ResultVO(200, "轮播图更换成功", true, null);
        } else {
            resultVO = new ResultVO(-1000, "轮播图更换失败", false, null);
        }
        return resultVO;
    }
}
